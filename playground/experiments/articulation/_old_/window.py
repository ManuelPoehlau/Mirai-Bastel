...
class PlaygroundWindow(pyglet.window.Window):
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int) -> None:
        ...
        # Axis: screen-right (camera.basis()[1]) — rotates "sideways from camera's perspective",
        # intuitive for a vertical mesh: drag right = lean right.
    ...
    def on_mouse_drag(
        ...
        if self._articulation_dragging and self._articulation_state is not None:
            angle = (x - self._articulation_press_x) * _ARTICULATION_SENSITIVITY
            self._articulation_state.update(angle)
            self._rebuild_vbo()
            return pyglet.event.EVENT_HANDLED
