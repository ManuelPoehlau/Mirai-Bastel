...
class ExtrudeGeometryTests(unittest.TestCase):
    ...
    def test_update_moves_vertices_along_normal(self):
        """update() verschiebt neue Vertices entlang der Normale."""
        self.tool.activate()
        self.tool.begin(face_id=self.face_id)
        mesh = self.scene.mesh
        # Simuliere Mausbewegung: dy=1.0 → Welt-Delta (0,1,0)
        # Projektion auf Normale (0,0,±1) → 0 (keine Z-Komponente)
        # → Wir brauchen ein Delta mit Z-Komponente
        self.tool.update(dx=0.0, dy=0.0, width=800, height=600)
        # Ohne Z-Komponente in camera stub → keine Bewegung
        # Aber der Test sollte die Projektion testen
        # → Stub gibt (dx,dy,0) zurück, Normale ist (0,0,±1)
        # → dot = 0, also keine Bewegung
        # Das ist korrekt für diesen Stub
    ...
    def test_extrude_distance_accumulates(self):
        ...
        # → total_distance bleibt 0 mit diesem Stub
        self.tool.update(dx=0.0, dy=0.0, width=800, height=600)
...
class ExtrudeCancelTests(unittest.TestCase):
    ...
    def test_cancel_restores_original_state(self):
        ...
        mesh = self.scene.mesh
        original_state = mesh.export_state()
        self.tool.activate()
        self.tool.begin(face_id=self.face_id)
        self.tool.cancel()
        self.tool.deactivate()
        # Mesh-Zustand ist identisch mit Original
        self.assertEqual(mesh.export_state(), original_state)
