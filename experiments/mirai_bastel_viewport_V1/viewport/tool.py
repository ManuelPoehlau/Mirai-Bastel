"""Minimales Tool-Framework für den Viewport-Praxistest (WP-02).
...
Bewusst klein und pyglet-frei (siehe WP-02 §4.2):
...
class ToolManager:
    """Minimaler Active-Tool-Mechanismus: höchstens ein interaktives Tool.

    Stellt sicher, dass beim Aktivieren/Zurückbauen kein alter Drag-/Tool-
    State hängen bleibt: Ein noch interagierendes Tool wird vor der
    Deaktivierung automatisch gecancelt (Geometrie-Restore, keine History).
    """
    ...
    # -- Lifecycle-Weiterleitung ------------------------------------------

    def activate(self, tool: Tool) -> None:
        """Setzt `tool` als einziges aktives Tool.

        Ein bereits aktives/interagierendes Tool wird zuvor sauber beendet
        (cancel falls interagierend, dann deactivate).
        """
    ...
    def begin(self, **params: Any) -> None:
        self._require_active().begin(**params)

    def update(self, **kwargs: Any) -> None:
        if not self.is_interacting:
            raise ToolStateError("update() ohne laufende Tool-Interaktion.")
        self._active.update(**kwargs)
