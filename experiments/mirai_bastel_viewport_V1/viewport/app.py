...
import pyglet
from pyglet.gl import GL_DEPTH_TEST, GL_LINES, GL_POINTS, GL_TRIANGLES, glEnable, glLineWidth, glPointSize
...
class ModelerWindow(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        with self.program:
            ...
            if self._hover_face_list is not None:
                self.program["color"] = (0.78,0.80,0.88); self._hover_face_list.draw(GL_TRIANGLES)
            ...
            if self._selected_face_list is not None:
                self.program["color"] = (1.0,0.55,0.15); self._selected_face_list.draw(GL_TRIANGLES)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        if button == mouse.LEFT:
            ...
            if picked is None:
                ...
                self.scene.selection.set({picked}); self._hovered_id = picked; self.scene.selection.hovered = picked
                if self.selection_mode == SelectionMode.VERTEX:
    ...
    def on_key_press(self,symbol,modifiers):
        if symbol in (key.V,key._1): self._set_selection_mode(SelectionMode.VERTEX)
            ...
            self._debug_face_pick = not self._debug_face_pick
            print(f"[FACE DEBUG] {'ON' if self._debug_face_pick else 'OFF'}")
