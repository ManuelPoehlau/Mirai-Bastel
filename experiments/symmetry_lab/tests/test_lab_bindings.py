"""Lab-Bindings (Handoff Slice 2 §4.5/§7, Slice 3 §4.1/§7) über das Production-`BindingSet`."""
...
def test_global_context_unchanged(bindings):
    ...
    probes = [ALT_LMB, SHIFT_LMB, LMB, MMB, RMB, WHEEL_UP, WHEEL_DOWN, SHIFT_S, Q, ESC, CTRL_Z]
    for inp in probes:
...
def test_symmetry_cycle_command_defined_in_lab_not_in_commands():
    assert SYMMETRY_CYCLE not in vars(cmd).values()


def test_shift_s_is_the_only_new_override():
    # Q/ESC/Ctrl+Z/Ctrl+Y sind globale Defaults (Fallback), keine Overrides.
    keys = [o for o in LAB_OVERRIDES if o.input.kind == "key"]
    assert [(o.input, o.command) for o in keys] == [(SHIFT_S, SYMMETRY_CYCLE)]
