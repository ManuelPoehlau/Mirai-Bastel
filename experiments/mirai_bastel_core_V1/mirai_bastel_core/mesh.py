...
class Mesh:
    ...
    def collapse_edge(self, edge_id: EdgeId) -> VertexId:
        ...
        # Edge-Lookup / Face-Referenzen für die verbleibenden Faces neu
        # aufbauen, da sich Vertex-Paare entlang der Boundaries geändert
        # haben.
        self._rebuild_edge_topology_for_vertex(survivor)
    ...
    def _rebuild_edge_topology_for_vertex(self, vertex_id: VertexId) -> None:
        """Interner Reparatur-Helfer nach collapse_edge.

        Kein Teil der öffentlichen Mutation-API - reine Implementierungs-
        Konsequenz der vereinfachten V1-Datenstruktur.
        """
        # Alte, jetzt evtl. inkonsistente Lookup-Einträge für betroffene
        # Faces entfernen und aus den aktuellen Boundaries neu aufbauen.
        stale_keys = [
            k for k, eid in self._edge_lookup.items() if vertex_id in k
        ]
        for k in stale_keys:
            eid = self._edge_lookup[k]
            e = self._edges.get(eid)
            if e is not None and (e.v0 == vertex_id or e.v1 == vertex_id):
                # nur Lookup bereinigen, Edge selbst bleibt ggf. gültig
                pass

        for fid, face in self._faces.items():
            if vertex_id not in face.boundary:
                continue
            n = len(face.boundary)
            for i in range(n):
                v_a, v_b = face.boundary[i], face.boundary[(i + 1) % n]
                ...
                if key not in self._edge_lookup:
                    eid = self._edge_alloc.allocate()
                    self._edges[eid] = _EdgeData(v0=v_a, v1=v_b, faces=[fid])
                    self._edge_lookup[key] = eid
                elif fid not in self._edges[self._edge_lookup[key]].faces:
                    self._edges[self._edge_lookup[key]].faces.append(fid)
