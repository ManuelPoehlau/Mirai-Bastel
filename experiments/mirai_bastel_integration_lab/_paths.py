...
(für die Production-Pakete `core`/`viewport`/`mirai`, WP-IL-01) und
`examples/` (für den geteilten OBJ-Loader, AD-007) werden auf `sys.path`
gelegt. Dadurch funktionieren alle Lab-Imports sowohl als Skript
(`python run.py`) als auch unter pytest.
...
def ensure_paths() -> None:
    """Legt Lab-Ordner, Repo-Root, Repo-`src/` und `examples/` auf sys.path."""
    for _path in (
        str(_LAB_DIR),
        str(_REPO_ROOT),
        str(_REPO_SRC_DIR),
